...
Y = [Y, I(:,1),trainingData(:,1001)]; %77220*7
...
for i = (1:size(testY,1))
    x = testY(i,:);
    result(i,1) = Gaussian(Lab1_mean, Lab1_var, x)*size(Lab1_Y,1)/size(Y,1);
    result(i,2) = Gaussian(Lab2_mean, Lab2_var, x)*size(Lab2_Y,1)/size(Y,1);
    result(i,3) = Gaussian(Lab3_mean, Lab3_var, x)*size(Lab3_Y,1)/size(Y,1);
end
